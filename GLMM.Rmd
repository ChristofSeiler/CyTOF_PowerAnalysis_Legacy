...
fake_data = function() {
  ...
  for(i in 1:(n_donors*n_cells)) {
    explanatory[i] = rnorm(n = 1,
                           mean = treatment[i]*shift_cell + explanatory_donor[donor[i]],
                           sd = sd_cell)
  }
  data.frame(treatment = factor(treatment,labels = c("LTNP","CP")),
             explanatory,
             donor = factor(donor))
}
test = fake_data()
...
power = sapply(1:n_sim,function(i) {
  cat(".")
  test = fake_data()
  res_glm = mhglm(treatment ~ explanatory + (explanatory | donor),
