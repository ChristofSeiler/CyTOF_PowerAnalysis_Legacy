...
fake_data = function(n_donors) {
  ...
  shift_cell = rep(0,p)
  shift_cell[select_markers] = 3*max(diag(Sigma))
  #sd_cell = 1
...
power_glm = function(n_donors) {
  ...
  res = mclapply(1:n_sim,function(i) {
    ...
    power = 1-type2
    c(type1,type2,power)
    }, mc.cores = n_cores) %>% simplify2array %>% t
  sink()
  avg = colMeans(res)
  c(Ns = n_donors,type1 = avg[1],type2 = avg[2],power = avg[3])
}
df_power = lapply(Ns,power_glm) %>% do.call(rbind,.) %>% data.frame
df_power = data.frame(df_power,fdr = alpha)
df_power_long = melt(df_power, id="Ns")
ggplot(data = df_power_long,aes(x = Ns,y = value, colour = variable)) + 
