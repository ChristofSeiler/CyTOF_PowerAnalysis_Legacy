...
Power as a function of ICC:

$$ \text{ICC} = \frac{\sigma_{\text{donor}}}{\sigma_{\text{donor}} + \sigma_{\text{cell}}} $$

We use the multivariate generalization GICC introduced by Yue et al. 2015[^1].
...
fake_data = function(ICC) {
  ...
  # effect size
  shift_cell = rep(0,p)
  sd_donor = sum(diag(Sigma)) * ICC/(1-ICC)
  #sd_cell = 1
...
test = fake_data(0.5)
ggplot(test, aes(status,PD1,color = donor)) + geom_point(position=position_jitter(w=0.3,h=0), size=4)
ggplot(test, aes(status,Granzyme_B,color = donor)) + geom_point(position=position_jitter(w=0.3,h=0), size=4)
ggplot(test, aes(status,CD19,color = donor)) + geom_point(position=position_jitter(w=0.3,h=0), size=4)
```
...
ggplot(data.frame(ICC = ICCs,power),aes(ICC,power)) + 
